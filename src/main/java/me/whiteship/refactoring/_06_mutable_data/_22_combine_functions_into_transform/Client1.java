 ...
 * - 관련있는 여러 파생 변수를 만들어내는 함수가 여러 곳에서 만들어지고 사용된다면
 *   그러한 파생 변수를 "변환 함수(transform function)"를 통해 한 곳으로 모아둘 수 있다.
 * - 소스 데이터가 변경될 수 있는 경우에는 "여러 함수를 클래스로 묶기(Combine Functions into Class)"를 사용하는 것이 적절하다.
 * - 소스 데이터가 변경되지 않는 경우에는 두 가지 방법을 모두 사용할 수 있지만,
...
public class Client1 extends ReadingClient{
    ...
    public Client1(Reading reading) {
        this.baseCharge = enrichReading(reading).basicCharge();
    }
