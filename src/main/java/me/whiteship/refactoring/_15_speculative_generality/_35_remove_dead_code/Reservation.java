 ...
 * - 나중에 이러저러한 기능이 생길 것으로 예상하여, 여러 경우에 필요로 할만한 기능을 만들어놨지만
 *   "그런 일은 없었고..." 결국에 쓰이지 않는 코드가 발생한 경우.
 *   - XP 의 YANGI(You aren't gonna need it) 원칙을 따르자.
 *   - 관련 리팩토링
 *      - 추상 클래스를 만들었지만 크게 유용하지 않다면 "계층 합치기(Collapse Hierarchy)"
 *      - 불필요한 위임은 "함수 인라인(Inline Function)" 또는 "클래스 인리인(Inline Class)"
 ...
 *      - 나중에 정말로 다시 필요해진다면 git 과 같은 버전 관리 시스템을 사용해 복원할 수 있다.
 */
